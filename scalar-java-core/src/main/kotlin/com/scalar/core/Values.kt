...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { ScalarInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ScalarInvalidDataException("`$name` is not set")
            is JsonNull -> throw ScalarInvalidDataException("`$name` is null")
            else -> throw ScalarInvalidDataException("`$name` is invalid, received $this")
        }
