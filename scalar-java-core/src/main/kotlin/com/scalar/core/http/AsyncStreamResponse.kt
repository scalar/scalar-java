...
// The bridge from a future-wrapped [StreamResponse] to the callback surface above. Nothing in an
// SDK without a streaming method reaches it — the reference publishes it unconditionally all the
// same, so an SDK that later gains one does not change the shape of `core.http`.
@JvmSynthetic
